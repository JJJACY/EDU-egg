...
class VersionController extends Controller {
  async all(){
    try{
      // let Versiondata = await this.ctx.model.Stacks.findAll().then(res=>{
  ...
  async insert() {
    ...
    try{
      await this.ctx.model.Version.create({
        project_id,name,sort,created_at
      })
      this.ctx.body={
        code: 200,
        message: '新增成功'
  ...
  async index(){
    ...
    try{
      let versionData = await this.ctx.model.Version.findAll({
        where:{project_id:id },
        order: [['sort','ASC']]
      }).then(res=>{
        return JSON.parse(JSON.stringify(res,null,4))
  ...
  async sort(){
    ...
    try{
      Promise.all(params.map(async (data,index)=>{
        await this.ctx.model.Version.update(
          {sort: index},
          { where:{id: data.id} })
      }))
      this.ctx.body={
  ...
  async delete(){
    let id = this.ctx.params.id;
    try{
      await this.ctx.model.Version.destroy( { where:{id} } ).then(res=>{
        console.log(res)
      })
      this.ctx.body ={
        code: 200,
